...
li.querySelector(".show-btn").addEventListener("click", () => {
  ...
  // Pulisce campo di ricerca per far riapparire tutta la lista aggiornata
  document.getElementById("searchSetlist").value = "";
});
...
document.getElementById("searchSetlist").addEventListener("input", function () {
  ...
  const scalettaLista = document.getElementById("scalettaLista");
  const nascostiContainer = document.getElementById("nascostiContainer");
  const nascostiOriginali = document.querySelectorAll("#scalettaNascosta li");
  const noResultsMsg = document.getElementById("noResultsMessage");
const anyVisible = [...document.querySelectorAll(
  mostraSoloNascosti ? "#scalettaNascosta li" : "#scalettaLista li"
)].some(li => li.style.display !== "none");

noResultsMsg.style.display = anyVisible ? "none" : "block";


  // Pulisce eventuali risultati duplicati precedenti
  scalettaLista.querySelectorAll(".hidden-search-clone").forEach(el => el.remove());

  if (mostraSoloNascosti) {
    nascostiOriginali.forEach(li => {
  ...
  // Normale modalità visibile: filtra i visibili
  document.querySelectorAll("#scalettaLista li:not(.hidden-search-clone)").forEach(li => {
    const text = li.textContent.toLowerCase();
    li.style.display = text.includes(query) ? "flex" : "none";
  });

  // 🔍 Cerca tra i nascosti e clona se corrisponde
  hiddenSongs.forEach(hiddenSong => {
    if (hiddenSong.toLowerCase().includes(query)) {
      const li = document.createElement("li");
      li.classList.add("hidden-song", "hidden-search-clone");

      li.innerHTML = `
        <div class="hidden-songs-grid">
          <span style="opacity: 0.6; font-style: italic;">${hiddenSong}</span>
          <button class="btn btn-small show-btn" title="Rendi visibile">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none"
              viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
              <path stroke-linecap="round" stroke-linejoin="round"
      ...
      // Associa evento anche al clone per "rendi visibile"
      li.querySelector(".show-btn").addEventListener("click", () => {
        moveToVisible(hiddenSong);
        // Rimuovi tutti i cloni se il brano è stato reso visibile
        scalettaLista.querySelectorAll(".hidden-search-clone").forEach(el => el.remove());
        // Pulisce anche il campo di ricerca per aggiornare la visualizzazione
        document.getElementById("searchSetlist").value = "";
      });
...
function apriMenuModifica(index, branoCorrente, utenteCorrente) {
  ...
  label.innerHTML = utenteCorrente ? `Prenotato da: <b>${utenteCorrente}</b>` : `<b>Nessun prenotato</b>`;
